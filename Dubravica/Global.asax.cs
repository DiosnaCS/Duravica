    ...
    public class MvcApplication : System.Web.HttpApplication
    {
            ...
            if (Context.Session != null)
            {
                if (Context.Session.IsNewSession)
                {
                    if (HttpContext.Current.Session.Count == 0)
                    {
                        ...
                        //Dubravica.Error.toFile("Session_Start hapened", this.GetType().Name.ToString());
                    }
                }
            }
        }
        ...
        protected void Session_OnEnd()
        {
            if (HttpContext.Current != null)
            {
                ...
                {
                    HttpContext.Current.Response.Redirect("~/Account/Login/");
                    Dubravica.Error.toFile("Session_onEnd hapened", this.GetType().Name.ToString());
                }
            }
            else {
                Dubravica.Error.toFile("Session_onEnd hapened with null current context", this.GetType().Name.ToString());
            }
        }
        ...
        protected void Application_Error()
        {
            ...
            string PathToErrorFile = Dubravica.Error.PathToErrorFile;
